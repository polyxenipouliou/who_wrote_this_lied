...
\begin{enumerate}
    ...
    \item \textbf{Systematic Comparison:} We present a comprehensive comparison of handcrafted features (12D, 55D) against pretrained transformer embeddings (768D MidiBERT) for composer classification, with results showing handcrafted features achieve 65.0\% balanced accuracy (peak ~70\% with feature selection) versus 45.3\% for MidiBERT with MLP on our 264-piece corpus.
    
    \item \textbf{Feature Importance Analysis:} Using Random Forest importance ranking and ANOVA, we identify the most discriminative features (note count, unison ratio, stepwise ratio) and provide musicological interpretation of why these features capture composer-specific patterns.
    
    \item \textbf{Velocity Feature Exclusion:} We explicitly exclude velocity features from our analysis due to concerns about editorial bias in MIDI transcriptions, ensuring that classification is based on composer-intentioned patterns rather than transcriber conventions.
...
\subsection{Handmade Feature Extension (55D)}

To complement the theory-driven 12D feature set, we extracted 55 additional statistical features across four musical dimensions: pitch features (f1--f10), rhythm/duration features (f16--f23), interval features (f24--f30, f52--f60), texture features (f47--f50), and higher-order statistics (f31--f34, f35--f46).

\subsubsection{Velocity Features (Excluded)}

Velocity features (f11--f15) were initially considered but excluded from final analysis. MIDI velocity values in symbolic datasets often reflect editorial conventions of score transcribers rather than composer intent, as historical scores from the Romantic era specify dynamics qualitatively (e.g., \textit{p}, \textit{f}) rather than as numerical values (1--127). To ensure our model captures genuine stylistic patterns rather than data source artifacts, we excluded velocity features from our primary analysis.
...
Handmade (55D, no vel) & 55 & \textbf{65.0\%} & 5.6\% \\
Handmade (21D, top) & 21 & \textbf{~70\%} & ~5\% \\
MidiBERT (768D) + MLP & 768 & 45.3\% & -- \\
\bottomrule
\end{tabular}
\end{table}

The 55D handcrafted feature set significantly outperforms MidiBERT with MLP, demonstrating that domain-specific features are more effective than general pretrained representations for this task.

\subsection{Per-Composer Performance}

Table~\ref{tab:percomposer} shows detailed metrics for the 55D feature set with SVM.

\begin{table}[h]
\centering
\caption{Per-composer classification metrics (55D features, SVM).}
\label{tab:percomposer}
\begin{tabular}{lrrrr}
\toprule
Composer & Precision & Recall & F1 & Support \\
\midrule
Schubert & 0.62 & 0.62 & 0.62 & 84 \\
Brahms & 0.69 & 0.64 & 0.66 & 109 \\
Schumann & 0.62 & 0.69 & 0.65 & 71 \\
\midrule
\textbf{Macro Avg} & \textbf{0.64} & \textbf{0.65} & \textbf{0.64} & \textbf{264} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Top 10 most important features for composer classification (55D, velocity excluded).}
\label{tab:importance}
\begin{tabular}{rlcl}
\toprule
Rank & Feature & Importance & Category \\
\midrule
1 & f1\_note\_count & 0.0505 & Pitch \\
2 & f27\_unison\_ratio & 0.0330 & Interval \\
3 & f28\_stepwise\_ratio & 0.0323 & Interval \\
4 & f22\_staccato\_ratio & 0.0315 & Rhythm \\
5 & f3\_pitch\_std & 0.0306 & Pitch \\
6 & f24\_interval\_mean & 0.0306 & Interval \\
7 & f4\_pitch\_range & 0.0305 & Pitch \\
8 & f34\_ioi\_skew & 0.0289 & Rhythm \\
9 & f5\_unique\_pitches & 0.0281 & Pitch \\
10 & f25\_interval\_std & 0.0270 & Interval \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:feature_curve} shows the relationship between number of features and classification accuracy. Peak performance (~70\%) is achieved with approximately 21 top-ranked features.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{feature_accuracy_curve_55.png}
\caption{SVM feature selection curve showing balanced accuracy as a function of number of top features. Peak performance (~70\%) achieved at ~21 features.}
\label{fig:feature_curve}
\end{figure}

\subsection{ANOVA Discriminability Analysis}

One-way ANOVA confirmed significant between-composer variance for multiple features ($p < 0.05$). Most discriminative: f1\_note\_count, f27\_unison\_ratio, f28\_stepwise\_ratio, and texture features (pt\_std, pt\_mean).

\subsection{MLP Classification Results}

We evaluated an MLP classifier as an additional baseline. Results show:

\begin{table}[h]
\centering
\caption{MLP vs SVM comparison.}
\label{tab:mlp}
\begin{tabular}{lcc}
\toprule
Model & Accuracy & Notes \\
\midrule
SVM (55D) & 65.0\% & RBF kernel, C=5.0 \\
MLP (768D MidiBERT) & 45.3\% & 2 hidden layers \\
\bottomrule
\end{tabular}
\end{table}

The MLP on MidiBERT embeddings underperformed compared to handcrafted features with SVM, suggesting that the pretrained representations do not capture Lieder-specific stylistic patterns effectively.
...
Schumann's staccato ratio (f22) and rhythm variance reflect his diverse articulation patterns and poetic declamation approach. Lower simultaneity suggests arpeggiated textures (as in \textit{Dichterliebe} opening). Complex rhythmic patterns mirror his expressive intensity.

\subsection{What Features Reveal About Brahms}

Brahms' high note count (f1) and pitch range (f4) reflect his symphonic piano writing and rich textures. Conservative interval patterns (f27\_unison\_ratio) support his characterization as classically restrained with folk song influence. Dense chordal writing captures his four-part, string-quartet-like piano voicing.
...
\subsection{Velocity Feature Exclusion Rationale}

We explicitly excluded velocity features from our analysis due to concerns about editorial bias in MIDI transcriptions. This decision strengthens our methodology by ensuring that classification is based on composer-intentioned patterns rather than transcriber conventions. The fact that our model achieves 65.0\% accuracy without velocity features demonstrates that genuine stylistic markers exist in other musical dimensions (pitch, rhythm, interval, texture).
...
\subsection{Feature Dominance Patterns}

Contrary to our initial hypotheses emphasizing harmonic features, note count (f1) and interval ratios (f27, f28) emerged as the most discriminative attributes. This suggests that for same-era composer classification, surface-level statistical regularities (piece length, melodic motion patterns) may carry more stylistic information than deeper tonal properties. This finding echoes Youngblood's (1958) original insight about statistical distributions capturing stylistic choice.

\section{Conclusion and Future Work}\label{sec:conclusion}

This exploratory study investigated whether computational features can capture individual composer style in German Lieder. Key findings: (1) Handcrafted features (55D, 65.0\%) outperform pretrained embeddings (45.3\%); (2) Note count and interval features are most discriminative; (3) Compact subsets (~21 features) achieve peak performance (~70\%); (4) Velocity features were excluded to avoid editorial bias.
