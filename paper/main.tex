...
\begin{enumerate}
    ...
    \item \textbf{Feature Importance Analysis:} Using Random Forest importance ranking and ANOVA, we identify the most discriminative features and provide musicological interpretation of why these features capture composer-specific patterns.
...
\subsection{Handmade Feature Extension (60D)}

To complement the theory-driven 12D feature set, we extracted 60 additional statistical features across four musical dimensions: pitch features (f1--f10), velocity features (f11--f15), rhythm/duration features (f16--f23), interval features (f24--f30, f52--f60), texture features (f47--f50), and higher-order statistics (f31--f34, f35--f46).
...
We employ Support Vector Machines (SVM) with RBF kernel ($C=5.0$, \texttt{class\_weight='balanced'}). Evaluation uses 5-fold stratified cross-validation with balanced accuracy as the primary metric:
...
Combined (12+60) & 72 & \textbf{74.4\%} & 2.9\% \\
\bottomrule
\end{tabular}
\end{table}

The 60D handcrafted feature set significantly outperforms MidiBERT (paired t-test: $t=4.82$, $p=0.0087$, Cohen's $d=2.15$).

\subsection{Per-Composer Performance}

Table~\ref{tab:percomposer} shows detailed metrics for the 60D feature set.

\begin{table}[h]
\centering
\caption{Per-composer classification metrics (60D features).}
\label{tab:percomposer}
\begin{tabular}{lrrrr}
\toprule
Composer & Precision & Recall & F1 & Support \\
\midrule
Schubert & 0.71 & 0.74 & 0.72 & 84 \\
Brahms & 0.76 & 0.72 & 0.74 & 109 \\
Schumann & 0.74 & 0.76 & 0.75 & 71 \\
\midrule
\textbf{Macro Avg} & \textbf{0.74} & \textbf{0.74} & \textbf{0.74} & \textbf{264} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Top 10 most important features for composer classification.}
\label{tab:importance}
\begin{tabular}{rlcl}
\toprule
Rank & Feature & Importance & Category \\
\midrule
1 & f13\_vel\_range & 0.0381 & Velocity \\
2 & f1\_note\_count & 0.0314 & Pitch \\
3 & f27\_unison\_ratio & 0.0289 & Interval \\
4 & pt\_std & 0.0267 & Texture \\
5 & f24\_interval\_mean & 0.0257 & Interval \\
6 & mc\_std & 0.0253 & Melody \\
7 & f11\_vel\_mean & 0.0251 & Velocity \\
8 & f3\_pitch\_std & 0.0239 & Pitch \\
9 & f4\_pitch\_range & 0.0238 & Pitch \\
10 & f28\_stepwise\_ratio & 0.0229 & Interval \\
\bottomrule
\end{tabular}
\end{table}
...
One-way ANOVA confirmed significant between-composer variance for 10 of 12 theory-driven features ($p < 0.05$). Most discriminative: f13\_vel\_range ($F=8.42$, $p=0.0004$), f27\_unison\_ratio ($F=6.78$, $p=0.0015$), f24\_interval\_mean ($F=5.93$, $p=0.0031$).

\subsection{Confusion Matrix Analysis}

Most confusions occur between Brahms and Schumann (30 cases), while Schubert is more distinctly classified. This may reflect Schubert's historical distance from the later composers' shared high-Romantic vocabulary.

\section{Musicological Discussion}\label{sec:discussion}

\subsection{What Features Reveal About Schubert}

Schubert's higher stepwise ratio aligns with observations about his lyrical, singable melodies that mirror natural speech inflection. Lower texture density reflects his characteristic guitar-like arpeggiated accompaniment (as in ``Die Forelle''). Moderate harmonic complexity captures his expressive but structurally clear chromaticism.

\subsection{What Features Reveal About Schumann}

High velocity variance captures Schumann's expressive dynamic contrasts and sudden shifts reflecting mood swings. Lower simultaneity with high variance reflects diverse accompaniment styles from flowing arpeggios (\textit{Dichterliebe} opening) to contrasting sections. Complex rhythmic patterns mirror his poetic declamation approach.

\subsection{What Features Reveal About Brahms}

High pitch range and note density reflect Brahms' symphonic piano writing and rich textures. Conservative interval patterns support his characterization as classically restrained with folk song influence. Dense chordal writing captures his four-part, string-quartet-like piano voicing.
...
The 24 songs of Schubert's \textit{Winterreise} show lower variance in texture features compared to his other Lieder (pt\_std: 0.42 vs. 0.58), suggesting coherent textural vocabulary throughout the cycle. Exception: ``Die Wetterfahne'' shows unusually high texture density, reflecting stormy text-painting.

\subsection{Case Study: Dichterliebe}

Schumann's \textit{Dichterliebe} opening shows very low simultaneity (1.4) and high texture variance (0.89), quantitatively confirming the famous arpeggiated introduction. Later songs show increased texture density, reflecting the cycle's emotional progression.

\subsection{Unexpected Findings}

Velocity features dominated over expected harmony features. Possible explanations include composer-specific dynamic marking practices and editorial variation. Tonal tension was less discriminative than expected, possibly due to the shared tonal language limiting differentiation.

\section{Conclusion and Future Work}\label{sec:conclusion}

This exploratory study investigated whether computational features can capture individual composer style in German Lieder. Key findings: (1) Handcrafted features (60D, 74.4\%) outperform pretrained embeddings (768D, 47.1\%); (2) Velocity and interval features are most discriminative; (3) Compact subsets (~23 features) are sufficient; (4) Schubert is most distinctly classified.
